...
const EduAddCategoryForm = (props) => {
  let { setOpen } = props;
  ...
  const createCategory = (e) => {
    ...
    let storageRef = ref(storage, "education-categories/" + timeNow.getTime());
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then(async (download) => {
          setIsUploading(false);
          setIsLoading(true);
          setDoc(doc(db, "education-categories", `${timeNow.getTime()}`), {
            id: timeNow.getTime(),
...
export default EduAddCategoryForm;
