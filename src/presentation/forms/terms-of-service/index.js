...
const UpdateTofSForm = () => {
  const location = useLocation();
  const history = useHistory();
  let { body } = location?.state;

  const [isLoading, setIsLoading] = React.useState(false);
  const [termsData, setTermsData] = React.useState(body);
  ...
  const updateTerms = async (e) => {
    ...
    const mRef = doc(db, "others", "terms");
    try {
  ...
  return (
    <div>
      ...
      <ValidatorForm onSubmit={updateTerms}>
        ...
        <QuillEditable setValue={setTermsData} value={termsData} />
        <br />
        <br />
        <Button
          type="submit"
